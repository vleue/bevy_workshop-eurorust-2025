...
use bevy::{
    camera::visibility::RenderLayers, prelude::*, scene::SceneInstanceReady,
...
pub fn dataset_plugin(app: &mut App) {
    app.add_plugins((PointCloudPlugin, LasLoaderPlugin))
        ...
        .add_systems(
            Update,
            load_pointcloud.run_if(on_timer(Duration::from_secs_f32(0.1))),
        );
}
...
fn load_pointcloud(
    ...
    let point_cloud = if dataset.loaded {
        dataset.point_clouds.get(play.current_frame).unwrap()
    } else {
        dataset.point_clouds.last().unwrap()
    };
